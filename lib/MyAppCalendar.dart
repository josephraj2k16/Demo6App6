...
void main() {
  initializeDateFormatting().then((_) => runApp(MyAppCalendar()));
}
...
class MyAppCalendar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: MyHomePage(title: 'Your Prescription Calendar'),
    );
  }
}

class MyHomePage extends StatefulWidget {
  MyHomePage({Key key,this.userId,this.auth, this.title}) : super(key: key);
...
//
  ...
  void initState() {
    ...
    int m = getEmail();
    print(m);
    subscription1=collectionReference.where("mrn",isEqualTo: m)//.where("start_date",isLessThanOrEqualTo: formatted)
...
//          )
  ...
  Widget _buildEventList2(){
    ...
    return ListView.builder(
      ...
      //(snapshot1.length)==0?new Text("No Data"):
...
//                .where('mrn', isEqualTo: m.toString())
//                .snapshots(),
//            builder:
